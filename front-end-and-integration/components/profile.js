...
export default function Profile() {
  ...
  const [userBotchainbalance, setuserBotchainbalance] = useState()
  ...
  useEffect(()=>{
    const getTheData = async() => {
      if(isConnected){
       ...
       try {
        const Botchainbalance = await ethersProvider.getBalance(address)
        const parseBotchainbalance = formatUnits(Botchainbalance, 18);
        console.log(parseBotchainbalance)
        setuserBotchainbalance(parseBotchainbalance)
  ...
  return (
    <div>
      ...
      <div className='mt-[1cm] profileinfo'>
       ...
       <div className='clear-both'>
        ...
        <div className='lg:float-right md:float-right mt-[0.1cm] font-[500]'>{soldBalance > 0 ? (<span>Total sales: {parseFloat(soldBalance).toFixed(6)} Botchain</span>) : (<span>Total sales: 0 Botchain</span>)}</div>
       </div>
        <div>
    ...
    <div className='mt-[0.8cm]'>
      ...
      (<div data-aos={aosValue} id="uploadFiles" className='mt-[1cm]' style={{border:"1px solid #333", backgroundImage:"url(/images/canon3.jpg)", backgroundRepeat:"no-repeat", backgroundPositionX:"50%"}}>
        ...
        <input type="number" className='p-[0.2cm] bg-[#001] rounded-md outline-[#fff] w-[100%] mb-[0.3cm]' value={itemPrice} onChange={(e) => setItemPrice(e.target.value)} placeholder="Set a price for your item e.g 1 Botchain" style={{border:"2px solid #00f"}} />
